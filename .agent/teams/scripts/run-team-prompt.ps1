...
$outputFile = Join-Path $runDir "output.txt"
...
$meta = [ordered]@{
    ...
    mode = $(if ($Interactive) { "interactive" } else { "print" })
...
if ($Interactive) {
    Write-Host "Launching interactive Claude session."
    Write-Host "When Claude opens, paste prompt from: $promptCopy"
    claude --model $Model
    exit $LASTEXITCODE
}

Write-Host "Launching non-interactive Claude execution..."
$promptText | claude -p --model $Model | Tee-Object -FilePath $outputFile
exit $LASTEXITCODE
