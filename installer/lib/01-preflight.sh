...
check_os() {
  ...
  if grep -qi microsoft /proc/version 2>/dev/null; then
    ...
    log_error "Kaanbal Engine requires Linux, WSL2, or macOS"
    exit 1
  fi
  ...
  if [ -f /etc/os-release ]; then
    ...
    KB_DISTRO="${ID:-unknown}"
    KB_DISTRO_VERSION="${VERSION_ID:-unknown}"
    log_info "Distro: ${PRETTY_NAME:-$KB_DISTRO $KB_DISTRO_VERSION}"
  fi
  ...
  case "$KB_ARCH" in
    ...
    *)
      log_error "Unsupported architecture: $KB_ARCH"
      exit 1
      ;;
  esac
  log_info "Architecture: $KB_ARCH"
...
check_resources() {
  ...
  if [ -z "$total_ram_kb" ] && [ "$KB_OS" = "macos" ]; then
